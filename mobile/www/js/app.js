...
const DB_CACHE_KEY = 'ssdec_erp_cache_v2';
...
function hydrateDBFromCache() {
  try {
    const raw = localStorage.getItem(DB_CACHE_KEY);
    if (!raw) return false;
    const parsed = JSON.parse(raw);
    Object.keys(DB).forEach(tbl => {
      DB[tbl] = Array.isArray(parsed?.[tbl]) ? parsed[tbl] : [];
    });
    return true;
  } catch (e) {
    return false;
...
async function loadAllData() {
  ...
  if (!successCount) {
    if (hydrateDBFromCache()) {
      toast('Offline mode: showing last synced data.', 'i');
      document.getElementById('offline-bar').style.display='block';
      setSyncB('err');
    } else {
      toast('Unable to load ERP data.', 'e');
    }
  } else {
    cacheDB();
  }
